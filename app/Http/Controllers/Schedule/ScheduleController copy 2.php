...
class ScheduleController extends BaseController
{
    ...
    public function getBookingTimes(Request $request)
    {
        ...
        $processedSlots = $this->processSlots($fetchData->slots, $dayOfWeek, $date, $breakTimeSlots);

        // dd($fetchData->slots->toArray());
        return view('admin.schedule-management.partials.booking_times', compact('fetchData', 'dayOfWeek', 'date', 'processedSlots','breakTimeSlots'));
    }

    private function processSlots($slots, $dayOfWeek, $date, $breakTimeSlots)
    {
        $processedSlots = [];
        foreach ($slots as $time) {
            if ($time->available_from !== null && $time->available_day === $dayOfWeek) {
                $processedSlots[] = $this->processSlot($time, $dayOfWeek, $date,$breakTimeSlots);
            }
        }
        return $processedSlots;
    }

    private function processSlot($time, $dayOfWeek, $date, $breakTimeSlots)
    {
        $totalBreakTime=$this->totalBreakTime($breakTimeSlots);
        $carbonStartTime = Carbon::parse($time->available_from);
        $carbonEndTime = Carbon::parse($time->available_to);
        $totalTimeInMinutes = $carbonEndTime->diffInMinutes($carbonStartTime);
        $interval = ($totalTimeInMinutes-$totalBreakTime) / $time->total_patient;
    ...
    {
        $break_from = Carbon::parse($breakTime->break_from);
        $break_to = Carbon::parse($breakTime->break_to);
        $totalTimeInMinutes = $break_to->diffInMinutes($break_from);
    ...
    private function totalBreakTime($breakTime)
    {
        $totalBreakTime = 0;
        foreach ($breakTime as $break) {
            ...
            // Calculate difference in minutes between break start time and break end time
            $difference = $breakFrom->diffInMinutes($breakTo);

            // Add the difference to the total break time
