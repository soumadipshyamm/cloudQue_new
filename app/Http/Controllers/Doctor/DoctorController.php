...
class DoctorController extends BaseController
{
    ...
    public function __construct(AuthContract $AuthContract, ClinicContract $ClinicContract, DoctorContract $DoctorContract,  PatientContract $PatientContract, CategoryContracts $CategoryContracts)
    {
        $this->AuthContract = $AuthContract;
        $this->ClinicContract = $ClinicContract;
        $this->DoctorContract = $DoctorContract;
        $this->PatientContract = $PatientContract;
        $this->CategoryContracts = $CategoryContracts;
    }
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                if($request->uuid == null){
                    ...
                    // $isDoctorCreated=$isDoctorCreated->id;
                    if(isset($request->categoryId)){
                        foreach($request->categoryId as $category){
                            $isCategory = $this->CategoryContracts->findCategoryById($category);
                            // dd($isCategory);
                            $isDoctorCreated->doctorsCategories()->attach($isCategory->id);
                        }
                    }
