            ...
            <table class="table table-hover dataTable">
                ...
                <tbody>
                    @forelse ($fetchData as $key=>$schedule)
                        {{-- @dd($schedule->clinices) --}}
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>
                                {{ $schedule->clinices->clinic_name ?? '' }}
                            </td>
                            {{-- <td>
                        {{ $schedule->users->name ?? '' }}
                    </td> --}}
                            <td>
                                <div class="action_box">
                                    @if (auth()->user()->hasRole('super-admin', 'clinic','doctor'))
                                        <a href="{{ route('admin.schedule.doctorScheduleList', $schedule->clinices->uuid) }}"
                                            class="text-primary"><i class="fa fa-calendar" aria-hidden="true"></i></a>
                                    @endif
                                    {{-- @if (auth()->user()->hasRole('doctor'))
                                        <a href="{{ route('admin.schedule.doctorScheduleSlot', [$schedule->clinices->id, $fetchDoctorId]) }}"
                                            class="text-primary"><i class="fa-regular fa-calendar-check"
